...
type ApiGetBookUserIdGetRequest struct {
	ctx context.Context
	ApiService *DefaultAPIService
	userId string
}

func (r ApiGetBookUserIdGetRequest) Execute() (*http.Response, error) {
	return r.ApiService.GetBookUserIdGetExecute(r)
}

/*
GetBookUserIdGet Get book information by user ID

Retrieve book information for a user based on the provided user ID.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param userId User ID
 @return ApiGetBookUserIdGetRequest
*/
func (a *DefaultAPIService) GetBookUserIdGet(ctx context.Context, userId string) ApiGetBookUserIdGetRequest {
	return ApiGetBookUserIdGetRequest{
		ApiService: a,
		ctx: ctx,
		userId: userId,
...
// Execute executes the request
func (a *DefaultAPIService) GetBookUserIdGetExecute(r ApiGetBookUserIdGetRequest) (*http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DefaultAPIService.GetBookUserIdGet")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/getBook/{userId}"
	localVarPath = strings.Replace(localVarPath, "{"+"userId"+"}", url.PathEscape(parameterValueToString(r.userId, "userId")), -1)
